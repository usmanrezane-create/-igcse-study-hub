...
// Premium payment window — PayPal Hosted Button supplied by the site owner.
const premiumDialog = document.querySelector('#premiumDialog');
const premiumClose = document.querySelector('#premiumClose');
const premiumButtons = [
  document.querySelector('#navPremiumButton'),
  document.querySelector('#heroPremiumButton')
].filter(Boolean);
let paypalHostedButtonRendered = false;

function renderHostedPayPalButton() {
  if (paypalHostedButtonRendered) return;
  const loading = document.querySelector('#paypalLoading');
  const container = document.querySelector('#paypal-container-SHCZMQMALAS5U');
  if (!container) return;

  if (!window.paypal?.HostedButtons) {
    if (loading) loading.textContent = 'PayPal could not load. Check your internet connection and refresh the page.';
    return;
  }

  window.paypal.HostedButtons({
    hostedButtonId: 'SHCZMQMALAS5U'
  }).render('#paypal-container-SHCZMQMALAS5U')
    .then(() => {
    ...
    .catch((error) => {
      ...
      if (loading) loading.textContent = 'PayPal checkout could not load. Refresh the page and try again.';
    });
}

function openPremiumDialog() {
  premiumDialog?.showModal();
  renderHostedPayPalButton();
}

premiumButtons.forEach((button) => button.addEventListener('click', openPremiumDialog));
premiumClose?.addEventListener('click', () => premiumDialog.close());
premiumDialog?.addEventListener('click', (event) => {
  const box = premiumDialog.getBoundingClientRect();
  const outside = event.clientX < box.left || event.clientX > box.right || event.clientY < box.top || event.clientY > box.bottom;
  if (outside) premiumDialog.close();
});
